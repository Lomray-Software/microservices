...
function fixture(enabled) {
  ...
  names.forEach((name, index) => rows.push(`# Subtest: ${name}`,
  ...
  for (const [key, value] of Object.entries({ tests: 3, suites: 0,
    pass: enabled ? 0 : 3, fail: enabled ? 1 : 0, cancelled: 0,
    skipped: enabled ? 2 : 0, todo: 0 })) rows.push(`# ${key} ${value}`);
...
const mutations = {
  ...
  'wrong skipped count': r => { r.stdout = r.stdout.replace('# skipped 2', '# skipped 1'); },
